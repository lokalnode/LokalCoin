...
Lokal Coin is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 2512 for connections and services them.
...
However, in open source software such as Lokal Coin, where everyone uses
...
pay attention to for reviewers of Lokal Coin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Lokal Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Lokal Coin so that they can be integrated
