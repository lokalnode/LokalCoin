...
*Setup instructions for a Gitian build of Lokal Coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lokal
...
- [Building Lokal Coin](#building-lokal-core)
...
Clone the git repositories for Lokal Coin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/lokalpay/lokal
...
in the Lokal Coin repository under 'Fetch and create inputs' to install sources which require
...
To build Lokal Coin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Lokal Coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lokal/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lokalpay/lokal
...
For example:
```bash
URL=https://github.com/crowning-/lokal.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit lokal=${COMMIT} --url lokal=${URL} ../lokal/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lokal=${COMMIT} --url lokal=${URL} ../lokal/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lokal=${COMMIT} --url lokal=${URL} ../lokal/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lokal git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lokal/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lokal
SIGPATH=/some/root/path/lokal-detached-sigs

./bin/gbuild --url lokal=${BTCPATH},signature=${SIGPATH} ../lokal/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lokal-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lokal-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lokal-osx-build.assert
```
...
[lokal/gitian.sigs](https://github.com/lokalpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
