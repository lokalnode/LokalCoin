...
# lokal_hash
RUN git clone https://github.com/lokalpay/lokal_hash
RUN cd lokal_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} lokal
RUN useradd -u ${USER_ID} -g lokal -s /bin/bash -m -d /lokal lokal
...
RUN mkdir /lokal-src && \
  ...
  chown $USER_ID:$GROUP_ID /lokal-src && \
