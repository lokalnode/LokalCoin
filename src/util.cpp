...
const char * const BITCOIN_CONF_FILENAME = "lokalcoin.conf";
const char * const BITCOIN_PID_FILENAME = "lokalcoin.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LOKALCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LOKALCoin
    // Mac: ~/Library/Application Support/LOKALCoin
    // Unix: ~/.lokalcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LOKALCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LOKALCoin";
#else
    // Unix
    return pathRet / ".lokalcoin";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty lokal.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override lokal.conf
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
		strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2018, nEndYear) + "The PACGlobal developers";
