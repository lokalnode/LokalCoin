...
static CMutableTransaction CreateProRegTx(SimpleUTXOMap& utxos, int port, const CScript& scriptPayout, const CKey& coinbaseKey, CKey& ownerKeyRet, CBLSSecretKey& operatorKeyRet)
{
    ...
    CAmount change;
    auto inputs = SelectUTXOs(utxos, Params().GetConsensus().nMasternodeCollateral, change);
    ...
    FundTransaction(tx, utxos, scriptPayout, Params().GetConsensus().nMasternodeCollateral, coinbaseKey);
