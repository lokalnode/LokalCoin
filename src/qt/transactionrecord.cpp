    ...
    if (nNet > 0 || wtx.IsCoinBase())
    {
        ...
        for(unsigned int i = 0; i < wtx.tx->vout.size(); i++)
        {
            ...
            if(mine)
            {
                ...
                {
                    // Received by LOKAL_Coin Address
...
void TransactionRecord::updateStatus(const CWalletTx &wtx, int chainLockHeight)
{
    ...
    {
        // The IsLockedByInstaLOKAL call is quite expensive, so we only do it when a state change is actually possible.
        if (status.lockedByChainLocks) {
            if (oldLockedByChainLocks != status.lockedByChainLocks) {
                status.lockedByInstantSend = wtx.IsLockedByInstaLOKAL();
            } else {
