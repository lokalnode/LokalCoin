...
{
    return QCoreApplication::translate("lokal-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in lokal.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in lokal.qrc)
...
/** Class encapsulating LOKAL_Coin startup and shutdown.
...
/** Main LOKAL_Coin application object */
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. LOKAL Coin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(lokal);
    Q_INIT_RESOURCE(lokal_locale);
    ...
    /// 6. Determine availability of data directory and parse lokal.conf
