...
static const std::map<ThemedColor, QColor> themedColors = {
    ...
    { ThemedColor::TX_STATUS_OPENUNTILDATE, QColor(64, 124, 255) },
    { ThemedColor::TX_STATUS_OFFLINE, QColor(192, 192, 192) },
    { ThemedColor::TX_STATUS_DANGER, QColor(200, 100, 100) },
    { ThemedColor::TX_STATUS_LOCKED, QColor(30, 47, 85) },
};
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a LOKAL Coin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no lokal: URI
    if(!uri.isValid() || uri.scheme() != QString("lokal"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::LOKAL, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert lokal:// to lokal:
    //
    //    Cannot handle this later, because pac:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("lokal://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "lokal:");
    }
...
{
    QString ret = QString("lokal:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::LOKAL, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open lokal.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "LOKAL_Coin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "LOKAL_Coin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("LOKAL_Coin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "LOKAL_Coin*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "lokalcore.desktop";
    return GetAutostartDir() / strprintf("lokalcore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a lokalcore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=LOKAL_Coin\n";
        else
            optionFile << strprintf("Name=LOKAL Coin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the LOKAL_Coin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add LOKAL_Coin app to startup item list
