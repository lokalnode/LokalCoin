...
#if defined(NDEBUG)
# error "LOKAL_Coin cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in LOKAL_Coin
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	if (nHeight <= Params().GetConsensus().nLastPoWBlock && nHeight > 0) {
		nSubsidy = 30000 * COIN;
	}
	...
	else if (nHeight <= 200000 && nHeight > 110000) {
		nSubsidy = 19.36 * COIN;
	}
	else if (nHeight <= 32487896 && nHeight > 200000) {
		nSubsidy = 12.10 * COIN;
	}
	else if (nHeight == 32487897) {
		nSubsidy = 10.40 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
void ThreadScriptCheck() {
    RenameThread("lokal_coin-scriptch");
...
static int64_t nTimeLOKAL_CoinSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // LOKAL : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(LOKAL): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(LOKAL): spork is off, skipping transaction locking checks\n");
    }
    ...
    // LOKAL : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (pindex->nHeight >= nEnforcedHeight)
    {
        if (!IsBlockValueValid(block, pindex->nHeight, blockReward, pindex->nMint, strError)) {
            return state.DoS(0, error("ConnectBlock(LOKAL): %s", strError), REJECT_INVALID, "bad-cb-amount");
        }
        ...
        if (!IsBlockPayeeValid(*block.vtx[1], pindex->nHeight, blockReward, pindex->nMint)) {
            return state.DoS(0, error("ConnectBlock(LOKAL): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
        }
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(LOKAL): ProcessSpecialTxsInBlock for block failed with %s", FormatStateMessage(state));
    }
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeLOKAL_CoinSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - LOKAL Coin specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeLOKAL_CoinSpecific * 0.000001);
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            if (state == THRESHOLD_ACTIVE || state == THRESHOLD_LOCKED_IN) {
                ...
                if (state == THRESHOLD_ACTIVE) {
                    //! this means little, and will just worry users
    ...
    std::string strMessage = strprintf("%s: new best=%s height=%d version=0x%08x log2_trust=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__,
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), chainActive.Tip()->nVersion,
      log(chainActive.Tip()->nChainTrust.getdouble())/log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast())
        return state.Invalid(false, REJECT_INVALID, "time-too-old", 
                                 strprintf("block's timestamp is too early %d %d", block.GetBlockTime(), pindexPrev->GetMedianTimePast()));
...
int ConfirmationsPerNetwork() {
    return (Params().NetworkIDString() ==
            CBaseChainParams::TESTNET ? 20 : 60);
}
