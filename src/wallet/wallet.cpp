...
DBErrors CWallet::ReorderTransactions()
{
    ...
    for (TxItems::iterator it = txByTime.begin(); it != txByTime.end(); ++it)
    {
        ...
        CAccountingEntry *const lokalentry = (*it).second.second;
        int64_t& nOrderPos = (pwtx != 0) ? pwtx->nOrderPos : lokalentry->nOrderPos;
        ...
        if (nOrderPos == -1)
        {
            ...
            if (pwtx)
            {
                ...
                if (!walletdb.WriteAccountingEntry(lokalentry->nEntryNo, *lokalentry))
                    return DB_LOAD_FAIL;
        }
        else
        {
...
bool CWallet::AbandonTransaction(const uint256& hashTx)
{
    ...
    if (origtx.GetDepthInMainChain() > 0 || origtx.InMempool() || origtx.IsLockedByInstaLOKAL()) {
        return false;
    }
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    for (std::pair<const uint256, CWalletTx>& item : mapWallet)
    {
        ...
        if (!wtx.IsCoinBase() && !wtx.IsCoinStake() && nDepth < 0 && !wtx.IsLockedByInstaLOKAL() && !wtx.isAbandoned()) {
...
CAmount CWallet::GetUnconfirmedBalance() const
{
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
        for (auto pcoin : GetSpendableTXs()) {
            if (!pcoin->IsTrusted() && pcoin->GetDepthInMainChain() == 0 && !pcoin->IsLockedByInstaLOKAL() && pcoin->InMempool())
                nTotal += pcoin->GetAvailableCredit();
        }
    }
    return nTotal;
    ...
    for (const auto& entry : mapWallet) {
        ...
        for (const CTxOut& out : wtx.tx->vout) {
            if (outgoing && IsChange(out)) {
                debit -= out.nValue;
            } else if (IsMine(out) & filter && (depth >= minDepth || (fAddLocked && wtx.IsLockedByInstaLOKAL())) && (!account || *account == GetAccountName(out.scriptPubKey))) {
                balance += out.nValue;
            }
        }
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, CoinType nCoinType) const
{
    ...
    for (unsigned int tryDenom = tryDenomStart; tryDenom < 2; tryDenom++)
    {
        ...
        for (const COutput &output : vCoins)
        {
            ...
            bool fLockedByIS = pcoin->IsLockedByInstaLOKAL();
...
bool CWallet::SelectPrivateCoins(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == Params().GetConsensus().nMasternodeCollateral) continue;
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (coin_control.nCoinType == CoinType::ONLY_DENOMINATED) {
                        ...
                        if (newTxOut.IsDust(MinRelayFee()))
                        {
                            nChangePosInOut = -1;
                            nFeeRet += nChange;
                ...
                if (nFeeNeeded < MinRelayFee().GetFee(nBytes))
                {
...
bool CWallet::CreateCoinStakeKernel(CScript &kernelScript, const CScript &stakeScript, unsigned int nBits, const CBlock &blockFrom, unsigned int nTxPrevOffset, const CTransactionRef &txPrev, const COutPoint &prevout, unsigned int &nTimeTx, bool fPrintProofOfStake) const
{
    ...
    {
        nTryTime = nTimeTx - i;
        if (CheckStakeKernelHash(nBits, chainActive.Tip(), blockFrom, nTxPrevOffset, txPrev, prevout, nTryTime, hashProofOfStake, true, false))
        {
...
bool CWallet::SelectStakeCoins(StakeCoinsSet &setCoins, CAmount nTargetAmount, const CScript &scriptFilterPubKey) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if (out.nDepth < (out.tx->tx->IsCoinStake() ? ConfirmationsPerNetwork() : 60))
            continue;
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances()
{
...

    {
        ...
        for (const auto& walletEntry : mapWallet)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if ((nDepth < (pcoin->IsFromMe(ISMINE_ALL) ? 0 : 1)) && !pcoin->IsLockedByInstaLOKAL())
                continue;
...
bool CWallet::ParameterInteraction()
{
    ...
    if (gArgs.IsArgSet("-paytxfee"))
    {
        ...
        if (MinTxFee() < MinRelayFee())
        {
            return InitError(strprintf(_("Invalid amount for -paytxfee=<amount>: '%s' (must be at least %s)"),
                                       gArgs.GetArg("-paytxfee", ""), MinRelayFee().ToString()));
        }
    }
...
bool CMerkleTx::IsLockedByInstaLOKAL() const
{
