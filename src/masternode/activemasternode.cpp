...
bool CActiveMasternodeManager::GetLocalAddress(CService& addrRet)
{
    ...
    if (LookupHost("8.8.8.8", addrDummyPeer, false)) {
        fFoundLocal = GetLocalMN(addrRet, &addrDummyPeer) && IsValidNetAddr(addrRet);
    }
    ...
    if (!fFoundLocal) {
        ...
        g_connman->ForEachNodeContinueIf(CConnman::AllNodes, [&fFoundLocal, &empty](CNode* pnode) {
            ...
            if (pnode->addr.IsIPv4() || pnode->addr.IsIPv6())
                fFoundLocal = GetLocalMN(activeMasternodeInfo.service, &pnode->addr) && IsValidNetAddr(activeMasternodeInfo.service);
            return !fFoundLocal;
        });
...
bool CActiveMasternodeManager::IsValidNetAddr(CService addrIn)
{
    ...
    return Params().NetworkIDString() == CBaseChainParams::REGTEST ||
           ((addrIn.IsIPv4() && IsReachable(addrIn) && addrIn.IsRoutable()) || addrIn.IsIPv6());
}
