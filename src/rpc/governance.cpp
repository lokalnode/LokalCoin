...
void gobject_prepare_help(CWallet* const pwallet)
{
    throw std::runtime_error(
                ...
                "5. use-IS        (boolean, optional, default=false) InstaLOKAL lock the collateral, only requiring one chain confirmation\n"
...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 LOKAL irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on lokalcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in lokal_coin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("lokal_coin.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in lokal_coin.conf\n"
#ifdef ENABLE_WALLET
...
UniValue getgovernanceinfo(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("proposalfee", ValueFromAmount(IsPoS() ? GOVERNANCE_PROPOSAL_FEE_TX_NEW : GOVERNANCE_PROPOSAL_FEE_TX_OLD)));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* LOKAL_Coin features */
    { "lokal",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "lokal",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "lokal",               "gobject",                &gobject,                true,  {} },
    { "lokal",               "voteraw",                &voteraw,                true,  {} },
