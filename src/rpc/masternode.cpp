...
void masternode_list_help()
{
    throw std::runtime_error(
            ...
            "  owneraddress     - Print the masternode owner LOKAL Coin address\n"
            "  payee            - Print the masternode payout LOKAL Coin address (can be additionally filtered,\n"
            "                     partial match)\n"
            "  pubKeyOperator   - Print the masternode operator public key\n"
            "  status           - Print masternode status: ENABLED / POSE_BANNED\n"
            "                     (can be additionally filtered, partial match)\n"
            "  votingaddress    - Print the masternode voting LOKAL Coin address\n"
        );
}
...
UniValue masternodelist(const JSONRPCRequest& request)
{
    ...
    mnList.ForEachMN(false, [&](const CDeterministicMNCPtr& dmn) {
        ...
        if (strMode == "addr") {
		...
		else if (strMode == "json" || strMode == "own") {
            std::ostringstream streamInfo;
			...
			objMN.push_back(Pair("banScore", dmn->pdmnState->nPoSePenalty));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "lokal",               "masternode",             &masternode,             true,  {} },
    { "lokal",               "masternodelist",         &masternodelist,         true,  {} },
};
