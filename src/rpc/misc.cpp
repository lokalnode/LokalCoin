...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total lokal coin balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"lokal_coin\" activates all LOKAL Coin-specific categories at once;\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "lokal_coin")
            + HelpExampleRpc("debug", "lokal_coin+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given lokal_coin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The lokal_coin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The lokal_coin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: LOKAL_Coin address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are lokal_coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) lokal_coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The lokal_coin address to use for the signature.\n"
...
UniValue logging(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "There are also a few meta-categories:\n"
            " - \"all\", \"1\" and \"\" activate all categories at once;\n"
            " - \"lokal_coin\" activates all LOKAL_Coin-specific categories at once.\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "util",               "setstaking",             &setstaking,             true,  {"mode"} },
    ...
    /* LOKAL_Coin features */
    { "lokal",               "mnsync",                 &mnsync,                 true,  {} },
    { "lokal",               "spork",                  &spork,                  true,  {"value"} },
