...
bool GetLocalMN(CService& addr, const CNetAddr *paddrPeer)
{
	int nBestScore = -1;
	int nBestReachability = -1;
	{
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Lokal users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "LOKAL Coin " + FormatFullVersion();
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    while (!interruptNet)
    {
        ...
        if (addrman.size() == 0 && (GetTime() - nStart > 7.5)) {
