...
#include "config/lokal_coin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("lokal_coin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via InstaLOKAL) in <address>"));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashinstantsenddoublespend=<address>", _("Enable publish transaction hashes of attempted InstaLOKAL double spend in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via InstaLOKAL) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawinstantsenddoublespend=<address>", _("Enable publish raw transactions of attempted InstaLOKAL double spend in <address>"));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all LOKAL Coin specific functionality (Masternodes, PrivateSend, InstaLOKAL, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<lokaladdress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
    ...
    strUsage += HelpMessageGroup(_("InstaLOKAL options:"));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/LOKAL_CoinOfficial/LOKAL>";
    const std::string URL_WEBSITE = "<https://lokalcoin.com/>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("lokal_coin-loadblk");
...
/** Sanity checks
 *  Ensure that LOKAL_Coin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LOKAL Coin version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single LOKAL_Coin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // lite mode disables all LOKAL_Coin-specific functionality
    fLiteMode = gArgs.GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, most Lokal-specific functionality is disabled."));
    }
    ...
    // ********************************************************* Step 10c: schedule LOKAL_Coin-specific tasks
    ...
    if(!fMasternodeMode && gArgs.GetBoolArg("-staking", true)) {
        threadGroup.create_thread(std::bind(&ThreadStakeMinter, boost::ref(chainparams), boost::ref(connman), boost::ref(vpwallets.front())));
    }
#endif
