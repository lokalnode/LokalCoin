...
// which are part of the Lokal repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="lokal-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /lokal-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "lokalpay-lokal/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from lokalpay-lokal/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'lokalpay-lokal/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from lokalpay-lokal/develop"
            }
          }

          if (hasCache) {
            sh "cd /lokal-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /lokal-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /lokal-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /lokal-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /lokal-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /lokal-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /lokal-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
